export function generateNavigationAuthenticated() {
  return `
    ...
    <li class="menu-item">
      <a href="#/add-info" class="flex items-center gap-2" aria-label="Tambah cerita baru">
...
export function generateNavigationUnauthenticated() {
  return `
    <li class="menu-item">
      <a href="#/" class="flex items-center gap-2" aria-label="Ke halaman Home">
        <i class="fas fa-home"></i> Beranda
      </a>
    </li>
    <li class="menu-item">
      <a href="#/about" class="flex items-center gap-2" aria-label="Ke halaman tentang">
    ...
    <div tabindex="0" data-storyid="${id}" class="card bg-base-100 shadow-xl hover:shadow-2xl transition-shadow duration-300 animate-fade-in" role="article" aria-labelledby="story-title-${id}">
      <div class="flex flex-col gap-2">
        <div class="relative w-full h-64 overflow-hidden rounded-t-lg">
          <img src="${photoUrl}" alt="Gambar cerita ${name}" class="h-64 w-full object-cover rounded-t-lg transition-transform duration-300 hover:scale-105"/>
        </div>
        <div class="card-body">
          ...
          <div class="flex items-start flex-wrap justify-between w-full">
            ...
            <p class="text-sm">${new Date(createdAt).toLocaleDateString("id-ID", {
              year: "numeric",
              month: "short",
              day: "numeric",
            })}</p>
          ...
          <div class="card-actions justify-end">
            <a class="btn btn-primary btn-sm" href="#/story/${id}" aria-label="Lihat detail cerita ${name}">
...
export const generatePopoutMap = ({ story }) => {
  return `
    <div class="flex flex-col gap-2 p-2 bg-base-100 rounded-lg shadow animate-fade-in" role="dialog" aria-label="Detail lokasi cerita ${story.name}">
      <p class="font-bold">${story.name}</p>
      <div class="relative w-32 h-32 sm:w-60 sm:h-60 overflow-hidden rounded-lg">
        <img src="${story.photoUrl}" alt="Gambar cerita ${story.name}" class="object-cover rounded-lg w-full h-full"/>
      </div>
