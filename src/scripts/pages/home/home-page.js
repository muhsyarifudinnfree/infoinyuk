...
export default class HomePage {
  ...
  async render() {
    return `
      <section class="container mx-auto px-4 py-12 animate-fade-in min-h-[80vh]" role="main" aria-label="Daftar semua cerita">
        <h1 class="text-2xl sm:text-3xl font-bold text-center mb-8">Semua Cerita</h1>
        <div id="map" class="h-80 sm:h-96 rounded-lg mb-8" role="region" aria-label="Peta lokasi cerita"></div>
        <div id="map-loading-container" class="hidden flex items-center justify-center h-80 sm:h-96 bg-base-200 rounded-lg">
        ...
        <div id="stories-loading-container" class="hidden flex items-center justify-center h-64">
          <span class="loading loading-spinner loading-lg" aria-label="Memuat daftar cerita"></span>
        </div>
      </section>
    `;
  }
